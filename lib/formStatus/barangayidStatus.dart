...
class _BarangayIDStatusPageState extends State<BarangayIDStatusPage> {
  ...
  Future<void> fetchBarangayIDRequests() async {
    final url = Uri.parse('https://manibaugparalaya.com/API/getBarangayID.php?user_id=${widget.userId}');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data is Map && data.containsKey("message")) {
          ...
          setState(() {
            barangayIDRequests = data;
            isLoading = false;
          });
        }
      } else {
        throw Exception("Failed to load Barangay ID requests");
      }
    } catch (error) {
