...
class _ClearanceStatusPageState extends State<ClearanceStatusPage> {
  ...
  Future<void> fetchClearanceRequests() async {
    final url1 = Uri.parse('https://manibaugparalaya.com/API/getClearance.php?user_id=${widget.userId}');
    final url2 = Uri.parse('https://manibaugparalaya.com/API/getClearance1.php?user_id=${widget.userId}');
    ...
    try {
      ...
      if (response1.statusCode == 200) {
        ...
        if (data1 is List) clearanceRequests.addAll(data1);
      }

      if (response2.statusCode == 200) {
        final data2 = json.decode(response2.body);
        print("Response2 Data: ${json.encode(data2)}");
        if (data2 is List) clearanceRequests.addAll(data2);
      }
  ...
  Future<void> _addNote(int index, int userId, int id, String note) async {
    final url = Uri.parse('https://manibaugparalaya.com/API/note6.php');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 'success') {
          setState(() {
            clearanceRequests[index]['note'] = note;
          });
  ...
  Widget _buildClearanceEntry(dynamic request, int index) {
      ...
      body: Padding(
        ...
        child: isLoading
          ...
          itemBuilder: (context, index) {
            return _buildClearanceEntry(clearanceRequests[index], index);
          },
        ),
      ),
    );
